...
# from https://github.com/mireshghallah/mixmatch/blob/main/mix_match_code/batched_MH/scripts/sample_batched.py
...
def get_opt_sent(sents,metadata):
    ...
    return " ".join(sent_best[1:-1]), meta_data[ind][-3],ind
...
def parallel_sequential_generation(
    ...
    for ii in range(max_iter):
        ...
        for positions in groups:
            ...
            if args.degenerate: 
                ...
                if args.theta:
                    org_emb = model_bert(batch)
                    #print(org_emb['hidden_states'][0].shape,org_emb['hidden_states'][1].shape)
                    print(org_emb[2][0].shape, org_emb[2][1].shape)
                    #print(org_emb)
                ...
                for i in range(batch_size):
                    full_meta_data[i].append( (sentiment, r_score[i],norm_score[i],qxxb[i],qxbx[i],axbx[i],acc[i].item(),disc_preds[i], distance[i] ,bleurt[i]))
    ...
    for i in range(batch_size):
        meta_data.append( (sentiment, r_score[i],norm_score[i],qxxb[i],qxbx[i],axbx[i],acc[i].item(),disc_preds[i] ,distance[i],bleurt[i]))
...
########
tokenizer_disc = BertTokenizer.from_pretrained(args.disc_dir) #BertTokenizer # finiteautomata/bertweet-base-sentiment-analysis ("textattack/bert-base-uncased-imdb")
