...
# from https://github.com/tinyvision/PreNAS/blob/main/model/supernet_transformer.py
...
from model.module.layernorm_super import LayerNormSuper, SwitchableLayerNormSuper
from model.module.scaling_super import ScalingSuper
...
class Vision_TransformerSuper(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 choices=None, switch_ln=False, scale_attn=False, scale_mlp=False, scale_embed=False):
        ...
        for i in range(depth):
            self.blocks.append(TransformerEncoderLayer(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio,
                                                       ...
                                                       choices=choices, switch_ln=switch_ln,
                                                       scale_attn=scale_attn, scale_mlp=scale_mlp, scale_embed=scale_embed))
        ...
        if scale_embed:
            self.embed_scaling = ScalingSuper(self.choices['embed_dim'])

        # self.pos_drop = nn.Dropout(p=drop_rate)
        if self.pre_norm:
            if switch_ln:
                self.norm = SwitchableLayerNormSuper(self.choices['embed_dim'])
            else:
    ...
    def set_sample_config(self, config: dict):
        ...
        if self.scale_embed:
            self.embed_scaling.set_sample_config(self.sample_embed_dim[0])
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, dropout=0., attn_drop=0.,
                 ...
                 relative_position=False, change_qkv=False, max_relative_position=14, choices=None, switch_ln=False,
                 scale_attn=False, scale_mlp=False, scale_embed=False):
        super().__init__()
        ...
        self.attn = AttentionSuper(
            ...
            max_relative_position=max_relative_position, choices=choices, scale_attn=scale_attn,
        )

        if switch_ln:
            self.attn_layer_norm = SwitchableLayerNormSuper(self.choices['embed_dim'])
            self.ffn_layer_norm = SwitchableLayerNormSuper(self.choices['embed_dim'])
        else:
            self.attn_layer_norm = LayerNormSuper(self.super_embed_dim)
            self.ffn_layer_norm = LayerNormSuper(self.super_embed_dim)

        if scale_embed:
            self.embed_scaling1 = ScalingSuper(self.choices['embed_dim'])
            self.embed_scaling2 = ScalingSuper(self.choices['embed_dim'])
        ...
        if scale_mlp:
            self.mlp_scaling = ScalingSuper([emb * ratio for emb in self.choices['embed_dim'] for ratio in self.choices['mlp_ratio']])
    ...
    def set_sample_config(self, is_identity_layer, sample_embed_dim=None, sample_mlp_ratio=None, sample_num_heads=None, sample_dropout=None, sample_attn_dropout=None, sample_out_dim=None):
        ...
        if self.scale_mlp:
            self.mlp_scaling.set_sample_config(self.sample_ffn_embed_dim_this_layer)
        ...
        if self.scale_embed:
            self.embed_scaling1.set_sample_config(sample_embed_dim=self.sample_embed_dim)
            self.embed_scaling2.set_sample_config(sample_embed_dim=self.sample_embed_dim)
    ...
    def forward(self, x):
        ...
        x = self.fc1(x)
        if self.scale_mlp:
            x = self.mlp_scaling(x)
        x = self.activation_fn(x)
        x = F.dropout(x, p=self.sample_dropout, training=self.training)
        x = self.fc2(x)
        if self.scale_embed:
            x = self.embed_scaling2(x)
        x = F.dropout(x, p=self.sample_dropout, training=self.training)
        if self.scale:
