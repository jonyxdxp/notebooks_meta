...
class SingleTurnDecoderNew(nn.Module):
    def __init__(self,
        ...
        self.mem_proj      = nn.Linear(context_dim, hidden_size)
    ...
    def forward(self, tgt_tokens, emb):
        ...
        causal_mask = nn.Transformer.generate_square_subsequent_mask(
            T, device=tgt_tokens.device).to(dtype=torch.bool)
    ...
    def generate(self, prompt_ids, z_fused, max_new_tokens=30,
        ...
        for _ in range(max_new_tokens):                          # ← loop start
            logits = self(generated, z_fused)[:, -1, :]         # ← inside

            if repetition_penalty != 1.0:                        # ← inside
            ...
            probs     = torch.softmax(logits / temperature, dim=-1)  # ← inside
            next_tok  = torch.multinomial(probs, 1)                  # ← inside
            generated = torch.cat([generated, next_tok], dim=1)      # ← inside

        return generated                                         # ← outside loop
